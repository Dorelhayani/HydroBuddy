...
void SendData(float temp, int light, int moisture) {
  HTTPClient http;
  String dataURL = "Temp=" + String(GetCurrentTemp());
  dataURL += "&Light=" + String(Getlight());
  dataURL += "&Moisture=" + String(Getmoisture());
...
void SendToDatasensors(float temp, int light, int moisture, bool isPumpON) {
  ...
  String dataToJson = "{\"temp\":" + String(GetCurrentTemp()) + ",\"light\":" + String(Getlight()) + ",\"moisture\":" + String(Getmoisture()) + ",\"isPumpON\":" + String(GetisPumpON() ? 1 : 0) + "}";
...
int GetState() {
  ...
  String STTS = String(GetCurrentStatus());
