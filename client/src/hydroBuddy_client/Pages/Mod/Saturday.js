...
export function HandleSaturday() {
    const [saturdayMode, setsaturdayMode] = useState(null); // אוחז רק את TEMP_MODE
    const [loading, setLoading] = useState(true);
    ...
    async function load() {
        try {
            ...
            const data = res?.SATURDAY_MODE || null;
            setsaturdayMode(data);
        } catch (e) {
    ...
    const fields = [
        { key: "dateAct", label: "Date" },
        { key: "timeAct", label: "Time (min)" },
        { key: "duration", label: "Time (min)" },
    ];

    function onLocalChange(key, raw) {
        setsaturdayMode((prev) => ({ ...prev, [key]: raw === "" ? "" : Number(raw) }));
    }
    ...
    async function patchSingle() {
        try {
            setSaving(true);
            setMsg("");
            await Api.saturday.saveChanges(saturdayMode);
        }
        catch (err) {
    ...
    if (!saturdayMode) return <p style={{ fontFamily: "Helvetica", color: "salmon" }}>No Data Found</p>;
    ...
    return (
        <div style={wrap}>
            ...
            {fields.map(({ key, label }) => (
                <div key={key} style={row}>
                    ...
                    <input
                        ...
                        type="text"
                        step="any"
                        value={saturdayMode[key]}
                        onChange={(e) => onLocalChange(key, e.target.value)}
                        style={input}
                    />
                    <button
                        onClick={() => patchSingle(key)}
                        disabled={saving || saturdayMode[key] === "" || Number.isNaN(Number(saturdayMode[key]))}
                        style={btnBlue}
            ...
            {saving && <p style={{ marginTop: 8 }}>Save…</p>}
